      ...
      <div class="card_container">
        <div v-for="juego in juegos" :key="juego.id">
          <div class="flex flex-col items-center mb-4">
            ...
            />
            <a
            class="mt-5 px-2 py-1 bg-[#1c1c1a] text-[#e2d9ca] rounded border border-[#7c7c72] 
                 hover:bg-[#72211d] hover:border-[#e2d9ca] hover:shadow-[0_0_15px_rgba(114,33,29,0.5)]
                 transition-all duration-500 text-xs uppercase tracking-wider relative overflow-hidden group/btn"
              :href="`/pdf/${juego.img_id}.pdf`"
              target="_blank"
            >
            <span class="relative z-10">Ver Reglamento</span>
            <span class="absolute inset-0 bg-gradient-to-r from-[#72211d]/80 to-[#72211d] transform translate-y-full group-hover/btn:translate-y-0 transition-transform duration-500"></span>
            </a>
          </div>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const showAlert = (gameName: string, gameId: number) => {
    ...
    alertMessage.value =
      index > -1
        ? `Se agregó el juego ${gameName} al carrito.`
        : `Se eliminó el juego ${gameName} del carrito.`;
